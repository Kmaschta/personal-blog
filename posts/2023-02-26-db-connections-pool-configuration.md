...
In a rush, we tried to blindly increase the maximum number of database connections as we were assuming the pool was maxing out.

This has no effect whatsoever because we didn't understand what was happening in the pool at the time.
...
[![Database Connection Lifecycle chart done on Stately Visualizer](/content/db-connections-pool-configuration/database-connection-lifecycle.png)](/content/db-connections-pool-configuration/database-connection-lifecycle.png)
...
Each system, infra, customers has its own specifities.
...
On the opposite, I'm recommending to get the following values as low as possible: `createTimeoutMillis`, `acquireTimeoutMillis`, and `destroyTimeoutMillis`. They are configured at 30 seconds by default for legacy reasons, but it's a really long time to wait to retry a connection.
